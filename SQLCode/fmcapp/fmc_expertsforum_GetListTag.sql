USE [fmcapp]
GO
/****** Object:  StoredProcedure [dbo].[fmc_expertsforum_GetListTag]    Script Date: 06/18/2013 18:05:13 ******/
SET ANSI_NULLS ON
GO
...
------------------------------------
ALTER PROCEDURE [dbo].[fmc_expertsforum_GetListTag]
 @page int
,@size int
,@expertid int
AS 
...
SELECT @pcount=count(1) FROM [fmc_expertsforum] 
...
IF(@page>1)
	BEGIN
		...
		SET ROWCOUNT @size
		SELECT @pcount,[ID],expertid,title,describe,smallimg,url,[uid],intime,click,deletetag
