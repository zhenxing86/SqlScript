DELIMITER $$
CREATE DEFINER=`zgyey_20`@`%` PROCEDURE `test`(IN `_uid` int,IN `_publicid` int,IN `_typeid` int)
BEGIN
...
/*
		SET @tablename = CASE(_uid % 100) % 10 WHEN 0 THEN 'push_message_00' 
																					 ...
																					 ELSE 'push_message'
											END;
		SET @tablename = CONCAT('kmapp_00.',  @tablename);
		...
		SET @sqlcmd = CONCAT('SELECT pmid, Case When msgtype = 3 Then 4 Else msgtype End contenttype, action, publicid,  
																 date_format(CASE WHEN publicid In (16, 17, 18) THEN adddate + INTERVAL 2 SECOND ELSE adddate END, ''%Y-%c-%d %H:%i:%s'') date,  
